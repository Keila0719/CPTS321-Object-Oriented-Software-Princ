...
namespace SpreadsheetEngine
{
    ...
    public class Spreadsheet
    {
        ...
        /// <summary>
        /// Check if the current cell name is withing the spreadsheet grid.
        /// </summary>
        /// <param name="cellName"> The current CellName it's checking.</param>
        /// <returns> True if the cell is within the range, otherwise false.</returns>
        private bool CheckWithinRange(string cellName)
        {
            // Check if the string is empty or not. If empty, return null.
            if (cellName == string.Empty)
            {
            ...
            // If they are not in bounds, return false
            return false;
        }
        ...
        private string? EvaluateCell(Cell cell)
        {
            ...
            {
                Cell? currentCell = this.GetCell(reference);

                if (currentCell != null)
                {
                    if (this.GetCellName(currentCell.RowIndex, currentCell.ColumnIndex) == currentCellName)
                    {
                        throw new SelfReferenceException("Exception: Self Reference");
                    }
                }
            }
            ...
            for (int i = 0; i < referenceList.Count; i++)
            {
                ...
                {
                    // Check if it's a non-existing cell reference
                    if (this.CheckWithinRange(currentCellName))
                    {
                        // We will need to treate the cell with no values or non numerical as 0.
                        expressionTree.SetVariable(referenceList[i], 0);
                    }
                    else
                    {
        ...
        private void Spreadsheet_PropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            ...
            {
                if (cell.Value != "#Circular Reference!" && cell.Value != "#Self Reference!")
                {
